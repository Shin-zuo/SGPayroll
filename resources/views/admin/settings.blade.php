...
<div class="p-6">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <i class="fa fa-cog text-slate-400"></i> Admin Settings
        </h2>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="col-span-1">
            <div class="bg-white rounded-lg border border-slate-100 shadow-sm p-5 text-center">
                ...
                <p class="text-slate-500 text-xs mb-3 flex items-center justify-center gap-1.5">
                    <i class="fa fa-envelope text-slate-400"></i> {{ $user->email }}
                </p>
                <div>
                    <span class="inline-block bg-blue-50 text-blue-700 text-xs px-2.5 py-1 rounded-md font-medium border border-blue-100">
                        {{ $user->user_type == 1 ? 'Administrator' : 'HR' }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="col-span-1 md:col-span-3">
            <div class="bg-white rounded-lg border border-slate-100 shadow-sm overflow-hidden">
                <div class="bg-slate-50/80 border-b border-slate-100 px-5 py-3 flex items-center">
                    <h4 class="text-sm font-semibold text-slate-800 m-0 uppercase tracking-wide">Update Settings</h4>
                </div>
                <div class="p-5">
                    <form action="/admin/settings" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-5">
                            <input type="file" id="profile_picture" name="profile_picture" class="form-control block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 border border-slate-200 rounded-md">
                        ...
                        <h5 class="text-sm font-semibold text-slate-700 mb-1.5 flex items-center gap-2">
                            <i class="fa fa-lock text-slate-400"></i> Change Password
                        </h5>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                            <div>
                                ...
                                <input type="password" id="password" name="password" class="w-full border border-slate-300 rounded-md p-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="Min. 6 characters">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5">Confirm New Password</label>
                                <input type="password" id="password_confirmation" name="password_confirmation" class="w-full border border-slate-300 rounded-md p-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
                            </div>
                        </div>

                        <div class="mt-5 pt-4 border-t border-slate-100 flex justify-end">
                            <button type="submit" id="saveSettingsBtn" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 px-5 rounded-md transition-colors flex items-center gap-2 shadow-sm">
