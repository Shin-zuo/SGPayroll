...
<table class="employee">
    ...
    <tr>
        ...
        <th rowspan="3" colspan="3">Holiday</th>
        <th rowspan="3" colspan="3">Holiday (minimum)</th>
        <th rowspan="3" colspan="3">Overtime</th>
        <th rowspan="3" colspan="3">Night differential</th>
        <th rowspan="3" colspan="3">Leave</th>
        <th rowspan="3" colspan="3">GROSS</th>
        <th colspan="4">GROSS COMPENSATION INCOME</th>
    ...
    <tr>
        ...
        <th colspan="1">TAXABLE</th>
    </tr>
    <tr>
        <th>13th Month</th>
        <th>SSS, PHIC & Pag-ibig Contribution</th>
        <th>De minimis benifits</th>
    ...
    <tr>
        ...
        <td colspan="3">{{number_format($employee->regular_holiday_amount+$employee->special_holiday_amount,2)}}</td>
        <td colspan="3">{{number_format($employee->regular_holiday_minimum_amount+$employee->special_holiday_minimum_amount,2)}}</td>
        <td colspan="3">{{number_format($employee->overtime_amount,2)}}</td>
        <td colspan="3">{{number_format($employee->night_diff_amount,2)}}</td>
        <td colspan="3">{{number_format($employee->vacation_leave_amount+$employee->sick_leave_amount,2)}}</td>
        <td colspan="3">{{number_format($employee->basic_pay+ 
        $employee->regular_holiday_amount+
        $employee->special_holiday_amount+
        $employee->regular_holiday_minimum_amount+
        $employee->special_holiday_minimum_amount+
        $employee->overtime_amount+
        $employee->night_diff_amount+
        $employee->vacation_leave_amount+
        $employee->sick_leave_amount+
        $employee->annual_non_tax_other+ 
        $employee->annual_cola_amount+
        $employee->thirteen_month+
        $employee->annual_sss+ 
        $employee->annual_phic+ 
        $employee->annual_hdmf,2)}}
    </td>
    @php
        $rest_day = $employee->rest_special_hours/8;
        $rest_day_total = $rest_day *  $employee->employee->basic_pay;
        if ($employee->salary_status) {
            $taxableIncome = number_format($employee->basic_pay+$employee->regular_holiday_minimum_amount+$employee->special_holiday_minimum_amount - ($employee->annual_sss + $employee->annual_phic + $employee->annual_hdmf),2);
        } else {
            $taxableIncome = number_format($employee->basic_pay + $employee->regular_holiday_amount+$employee->special_holiday_amount+$employee->overtime_amount+$employee->night_diff_amount+$employee->vacation_leave_amount+$employee->sick_leave_amount,2);
        }
    @endphp
        <td>{{$employee->thirteen_month}}</td>
    ...
    <tr>
        ...
        <td colspan="3"><strong>{{number_format($employees->sum('regular_holiday_amount')+$employees->sum('special_holiday_amount'),2)}}</strong></td>
        <td colspan="3"><strong>{{number_format($employees->sum('regular_holiday_minimum_amount')+$employees->sum('special_holiday_minimum_amount'),2)}}</strong></td>
        ...
        <td colspan="3"><strong>{{number_format($employees->sum('vacation_leave_amount')+$employees->sum('sick_leave_amount'),2)}}</strong></td>
        <td colspan="3"><strong>{{number_format($employees->sum('basic_pay')+$employees->sum('regular_holiday_amount')+$employees->sum('special_holiday_amount')+$employees->sum('overtime_amount')+$employees->sum('night_diff_amount')+$employees->sum('vacation_leave_amount')+$employees->sum('sick_leave_amount')+$employees->sum('thirteen_month')+$employees->sum('annual_sss')+$employees->sum('annual_phic')+$employees->sum('annual_hdmf')+$employees->sum('annual_non_tax_other') + $employees->sum('annual_cole_amount'),2)}}</strong></td>
        <td><strong>{{number_format($employees->sum('thirteen_month'),2)}}</strong></td>
        <td><strong>{{number_format($employees->sum('annual_sss')+$employees->sum('annual_phic')+$employees->sum('annual_hdmf'),2)}}</strong></td>
        <td><strong>{{number_format($employees->sum('annual_non_tax_other') + $employees->sum('annual_cola_amount'),2)}}</strong></td>
        <td><strong>{{number_format($employees->sum('basic_pay')+$employees->sum('regular_holiday_amount')+$employees->sum('special_holiday_amount')+$employees->sum('overtime_amount')+$employees->sum('night_diff_amount')+$employees->sum('vacation_leave_amount')+$employees->sum('sick_leave_amount'),2)}}</strong></td>
...
<div class="page_break">
    ...
    <table class="employee">
        ...
        <th rowspan="3" colspan="3">Basic Salary</th>
        <th rowspan="3" colspan="3">Holiday</th>
        <th rowspan="3" colspan="3">Holiday (minumum)</th>
        <th rowspan="3" colspan="3">Overtime</th>
        <th rowspan="3" colspan="3">Night differential</th>
        <th rowspan="3" colspan="3">Leave</th>
        <th rowspan="3" colspan="3">GROSS</th>
    ...
    <tr>
        ...
        <td colspan="3">{{number_format($inactive_employee->regular_holiday_amount+$inactive_employee->special_holiday_amount,2)}}</td>
        <td colspan="3">{{number_format($inactive_employee->regular_holiday_minimum_amount+$inactive_employee->special_holiday_minimum_amount,2)}}</td>
        ...
        <td colspan="3">{{number_format($inactive_employee->vacation_leave_amount+$inactive_employee->sick_leave_amount,2)}}</td>
        <td colspan="3">{{number_format($inactive_employee->basic_pay + $inactive_employee->regular_holiday_amount+$inactive_employee->regular_holiday_minimum_amount+$inactive_employee->special_holiday_minimum_amount+$inactive_employee->special_holiday_amount+$inactive_employee->overtime_amount+$inactive_employee->night_diff_amount+$inactive_employee->vacation_leave_amount+$inactive_employee->sick_leave_amount+$inactive_employee->annual_non_tax_other + $inactive_employee->annual_cola_amount+$inactive_employee->thirteen_month+$inactive_employee->annual_sss + $inactive_employee->annual_phic + $inactive_employee->annual_hdmf,2)}}</td>
                @php
                ...
                <td>{{number_format($employee->basic_pay + $employee->regular_holiday_amount+$employee->special_holiday_amount+$employee->overtime_amount+$employee->night_diff_amount+$employee->vacation_leave_amount+$employee->sick_leave_amount,2)}}</td>
                @php
                    $taxDue = 0;
                    $dues = 0;
                    $refund = 0;
        ...
        <td colspan="3"><strong>{{number_format($inactive_employees->sum('regular_holiday_amount')+$inactive_employees->sum('special_holiday_amount'),2)}}</strong></td>
        <td colspan="3"><strong>{{number_format($inactive_employees->sum('regular_holiday_minimum_amount')+$inactive_employees->sum('special_holiday_minimum_amount'),2)}}</strong></td>
        ...
        <td colspan="3"><strong>{{number_format($inactive_employees->sum('vacation_leave_amount')+$inactive_employees->sum('sick_leave_amount'),2)}}</strong></td>
        <td colspan="3"><strong>{{number_format($inactive_employees->sum('basic_pay')+$inactive_employees->sum('regular_holiday_amount')+$inactive_employees->sum('special_holiday_amount')+$inactive_employees->sum('overtime_amount')+$inactive_employees->sum('night_diff_amount')+$inactive_employees->sum('vacation_leave_amount')+$inactive_employees->sum('sick_leave_amount')+$inactive_employees->sum('thirteen_month')+$inactive_employees->sum('annual_sss')+$inactive_employees->sum('annual_phic')+$inactive_employees->sum('annual_hdmf')+$inactive_employees->sum('annual_non_tax_other') + $inactive_employees->sum('annual_cole_amount'),2)}}</strong></td>
